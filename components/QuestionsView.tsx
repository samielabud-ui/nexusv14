...
const QuestionsView: React.FC<QuestionsViewProps> = ({ userStats, onNavigateToPremium, onIncrementUsage, onAddActivity }) => {
  ...
  const handleGenerate = () => {
    ...
    onAddActivity({
      ...
      subtitle: `${moduloTitle} • ${quantidade} itens`
    });
  ...
  if (isGenerated) {
    return (
      <div className="max-w-[1600px] mx-auto animate-in fade-in duration-500">
        <header className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-neutral-800 pb-8">
          <div>
            <div className="flex items-center flex-wrap gap-2 md:gap-4 mb-2">
              <span className="text-[9px] md:text-[10px] font-black text-blue-500 uppercase tracking-[0.2em]">{ciclo}</span>
              <span className="hidden md:inline text-neutral-700">/</span>
              <span className="text-[9px] md:text-[10px] font-black text-neutral-500 uppercase tracking-[0.2em] truncate max-w-[200px] md:max-w-[400px]">{moduloTitle}</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">Prática de Questões</h2>
          </div>
          <div className="flex gap-3">
             <button onClick={() => setIsGenerated(false)} className="w-full md:w-auto text-[10px] font-bold text-neutral-400 hover:text-white flex items-center justify-center gap-2 transition-colors bg-neutral-900 px-5 py-3 rounded-xl border border-neutral-800 uppercase tracking-widest">
              Configurar Sessão
            </button>
          </div>
        </header>
            ...
            {filteredQuestions.length > 0 ? (
              <>
                ...
                <div className="py-12 md:py-20 text-center">
                  <button onClick={resetModule} className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white px-10 py-4 rounded-2xl text-md font-black transition-all uppercase tracking-widest">
                    Finalizar e Voltar
                  </button>
                </div>
              </>
            ) : (
              <div className="py-20 md:py-40 text-center border border-dashed border-neutral-800 rounded-[2rem] md:rounded-[3rem]">
                <h3 className="text-white text-xl md:text-2xl font-bold mb-4 px-4">Sem questões para os filtros aplicados</h3>
                <button onClick={() => setIsGenerated(false)} className="text-blue-500 font-black uppercase tracking-widest text-xs">Ajustar Filtros</button>
              </div>
            )}
          </div>
          ...
          <aside className="lg:col-span-3">
            <div className="lg:sticky lg:top-24 space-y-4">
              <div className="bg-neutral-900/80 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-2xl">
                <h3 className="text-[10px] font-black text-neutral-500 uppercase tracking-[0.2em] mb-4 text-center">Progresso Local</h3>
                <div className="flex items-center justify-between">
                  <div className="text-center">
                    <p className="text-[10px] text-emerald-500 font-bold uppercase">Acertos</p>
                    <p className="text-xl md:text-2xl font-black text-emerald-500">{Object.values(sessionAnswers).filter(s => s === 'correct').length}</p>
                  </div>
                  <div className="h-10 w-px bg-neutral-800"></div>
                  <div className="text-center">
                    <p className="text-[10px] text-red-500 font-bold uppercase">Erros</p>
                    <p className="text-xl md:text-2xl font-black text-red-500">{Object.values(sessionAnswers).filter(s => s === 'incorrect').length}</p>
                  </div>
                </div>
              </div>
            </div>
  ...
  if (selectedModuleId) {
    return (
      <div className="max-w-[1000px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
        ...
        <button onClick={resetModule} className="mb-8 flex items-center gap-2 text-neutral-500 hover:text-white transition-colors group">
        ...
        <div className="bg-neutral-900/30 border border-neutral-800 p-6 md:p-16 rounded-[2rem] md:rounded-[3rem] shadow-2xl relative overflow-hidden">
          <header className="mb-10 md:mb-12 text-center">
            <span className="text-[9px] md:text-[10px] font-black text-blue-500 uppercase tracking-[0.3em] block mb-2">{ciclo}</span>
            <h2 className="text-2xl md:text-3xl font-black text-white tracking-tighter px-2">{moduloTitle}</h2>
              ...
              <div className="mt-8 space-y-3 px-2">
                <div className="flex flex-col sm:flex-row justify-center gap-3">
                  <button 
                    ...
                    disabled={exporting}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-white hover:bg-neutral-200 text-black px-6 py-3 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all shadow-xl disabled:opacity-50"
                  >
                  ...
                  <button 
                    ...
                    disabled={exporting}
                    className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all shadow-xl disabled:opacity-50 ${userStats?.isPremium ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-neutral-800 text-neutral-500 border border-neutral-700'}`}
                  >
                    {!userStats?.isPremium ? (
          ...
          <div className="space-y-8 md:space-y-10">
            <section>
              <label className="block text-[10px] font-black text-neutral-500 uppercase tracking-[0.2em] mb-4">Quantidade de Questões</label>
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 md:gap-3">
                {['5', '10', '20', '30', 'Todas'].map((q) => (
                  <button key={q} onClick={() => setQuantidade(q)} className={`py-3 md:py-4 rounded-xl border text-[10px] md:text-xs font-black transition-all uppercase tracking-widest ${quantidade === q ? 'bg-blue-600 border-blue-600 text-white shadow-xl' : 'bg-neutral-950 border-neutral-800 text-neutral-500 hover:border-neutral-700'}`}>
                    {q}
                  </button>
                ))}
              </div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select value={modalidade} onChange={(e) => setModalidade(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl py-3 md:py-4 px-4 md:px-6 text-xs md:text-sm text-white focus:border-blue-600 outline-none cursor-pointer">
              ...
              <div>
                <label className="block text-[10px] font-black text-neutral-600 uppercase tracking-widest mb-3">Problema</label>
                <select value={problema} onChange={(e) => setProblema(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl py-3 md:py-4 px-4 md:px-6 text-xs md:text-sm text-white focus:border-blue-600 outline-none cursor-pointer">
            ...
            <section>
              <label className="block text-[10px] font-black text-neutral-600 uppercase tracking-widest mb-3">Palavra-chave do Tema</label>
              <input type="text" placeholder="Ex: Anatomia, Fisiologia..." className="w-full bg-neutral-950 border border-neutral-800 rounded-xl py-3 md:py-4 px-6 md:px-8 text-xs md:text-sm text-white focus:border-blue-600 outline-none placeholder:text-neutral-800" value={tema} onChange={(e) => setTema(e.target.value)} />
            </section>

            <div className="pt-6 border-t border-neutral-800">
              <button onClick={handleGenerate} disabled={isOverLimit} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 md:py-6 rounded-2xl transition-all shadow-2xl shadow-blue-600/20 text-sm md:text-lg uppercase tracking-widest flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3 disabled:opacity-50">
  ...
  return (
    <div className="max-w-[1800px] mx-auto px-0 md:px-4 pb-32 animate-in fade-in duration-500">
      <header className="mb-10 md:mb-16 px-4">
        <h2 className="text-4xl md:text-6xl font-black text-white mb-4 md:mb-6 tracking-tighter italic">Questões</h2>
        <p className="text-neutral-400 text-lg md:text-2xl font-light max-w-4xl leading-relaxed">Selecione um módulo para praticar questões baseadas no método PBL e morfofuncional.</p>
      </header>

      <div className="space-y-12 md:space-y-16">
        <section className="px-4">
          <h3 className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-6 flex items-center gap-4">
             Ciclo Básico <div className="h-px flex-grow bg-blue-500/20"></div>
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
            {modulesBasico.map((m) => (
              <div key={m.id} onClick={() => handleModuleSelect(m, 'Ciclo Básico')} className={`bg-neutral-900/30 border-2 ${m.color} p-6 md:p-8 rounded-2xl md:rounded-3xl cursor-pointer hover:bg-neutral-900 hover:scale-[1.02] transition-all flex flex-col justify-between h-44 md:h-52 group shadow-lg`}>
                ...
                <span className="text-[9px] md:text-[10px] font-bold uppercase text-neutral-500 group-hover:text-blue-500 transition-colors">Praticar Questões →</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
            {modulesClinico.map((m) => (
              <div key={m.id} onClick={() => handleModuleSelect(m, 'Ciclo Clínico')} className={`bg-neutral-900/30 border-2 ${m.color} p-6 md:p-8 rounded-2xl md:rounded-3xl cursor-pointer hover:bg-neutral-900 hover:scale-[1.02] transition-all flex flex-col justify-between h-44 md:h-52 group shadow-lg`}>
                <span className="text-[9px] md:text-[10px] font-black text-indigo-400 uppercase tracking-widest">ASE {m.id}</span>
                <h4 className="text-base md:text-lg font-bold text-neutral-100 leading-tight group-hover:text-white">{m.title}</h4>
                <span className="text-[9px] md:text-[10px] font-bold uppercase text-neutral-500 group-hover:text-indigo-500 transition-colors">Praticar Questões →</span>
              </div>
            ))}
          </div>
        </section>
